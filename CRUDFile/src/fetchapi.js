...
// Fetch the list of all brands from the API
fetch('http://makeup-api.herokuapp.com/api/v1/products.json')
    .then((response) => response.json())
    .then((data) => {
        var brandList = document.getElementById('brandList');
        var allBrands = data.map(item => item.brand);
        var uniqueBrands = allBrands.filter((value, index, self) => self.indexOf(value) === index);
...
function buttonClicked() {
    var searchTerm = document.getElementById("brandList").value;

    var apiUrl = `http://makeup-api.herokuapp.com/api/v1/products.json?brand=${searchTerm}`;
    if (!searchTerm) {
        console.error('Please provide a brand name for the search.');
        return;
    }
